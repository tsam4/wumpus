#ifndef PRLITE_COLOURMAP_HPP
#define PRLITE_COLOURMAP_HPP
 ...
 * (eg colourRangeWCBM) available from prlite_colourmap.hpp here as
 * function parameter.
 *
...
gLinear::gRowMatrix<float>
colourMap(
  Lambda&& colourRange,
  unsigned sz,
  float warp) {

  gLinear::gRowMatrix<float> cmap(sz,3);
...
std::vector< gLinear::gRowMatrix<float> >
colourmapImage(
  const gLinear::gRowMatrix<float>& pMat,
  Lambda&& colourRange,
  ...
  std::vector< gLinear::gRowMatrix<float> >  cmapImage(3);
...
std::vector< gLinear::gRowMatrix<float> >
overlayImage(
  const std::vector< gLinear::gRowMatrix<float> > & backdrop,
  const gLinear::gRowMatrix<float>& ovl,
  Lambda&& colourRange,
  bool normalize = false,
  float thresh = 0.0,
  float warp = 1.0) {
  ...
  std::vector< gLinear::gRowMatrix<float> > bgr(nChans);
